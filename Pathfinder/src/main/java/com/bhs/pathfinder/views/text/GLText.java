...
package com.bhs.pathfinder.views.text;
...
import com.bhs.pathfinder.views.text.programs.BatchTextProgram;
import com.bhs.pathfinder.views.text.programs.Program;
...
public class GLText {
	...
	public void draw(String text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ)  {
		...
		for (int i = 0; i < len; i++)  {              // FOR Each Character in String
			...
			batch.drawSprite(letterX, letterY, chrWidth, chrHeight, charRgn[c], modelMatrix, z);  // Draw the Character
	...
	public void drawTexture(int width, int height, float[] vpMatrix)  {
		...
		batch.drawSprite(width - (textureSize / 2), height - ( textureSize / 2 ), 
				textureSize, textureSize, textureRgn, idMatrix, 0);  // Draw
		batch.endBatch();                               // End Batch
	}
}
